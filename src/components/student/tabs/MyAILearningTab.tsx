...
import { Video, Headphones, HelpCircle, Brain, Sparkles } from "lucide-react";
...
export const MyAILearningTab = ({ student }: MyAILearningTabProps) => {
  const hasVideoData = (student.ai_video_usage?.total_videos || 0) > 0;
  const hasPodcastData = (student.podcast_usage?.total_listened || 0) > 0;
  const hasDoubtData = (student.doubt_clearing?.total_doubts || 0) > 0;
  const hasAnyData = hasVideoData || hasPodcastData || hasDoubtData || student.ai_queries > 0;

  // Video completion distribution
  const videoCompletionData = hasVideoData ? [
  ...
  // Empty state for no AI learning data
  if (!hasAnyData) {
    return (
      <Card>
        <CardContent className="py-12">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">Start Your AI Learning Journey</h3>
            <p className="text-muted-foreground mb-4">
              Use AI videos, podcasts, and the doubt clearing feature to see your learning analytics here.
            </p>
            <Link to="/dashboard">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{student.ai_video_usage?.watched_count || 0}</div>
            <p className="text-xs text-muted-foreground">of {student.ai_video_usage?.total_videos || 0}</p>
            {hasVideoData && (
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Math.round((student.ai_video_usage?.total_watch_time_minutes || 0) / 60)}h
            </div>
            <p className="text-xs text-muted-foreground">
              {student.ai_video_usage?.total_watch_time_minutes || 0} minutes
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{student.podcast_usage?.total_listened || 0}</div>
            <p className="text-xs text-muted-foreground">
              {Math.round((student.podcast_usage?.total_time_minutes || 0) / 60)}h listened
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{student.ai_queries || 0}</div>
      ...
      {hasVideoData && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              {(student.ai_video_usage?.recent_videos?.length || 0) > 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  ...
                  <span className="text-xl font-bold">{student.doubt_clearing?.total_doubts || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  <span className="text-sm">Resolved</span>
                  <span className="text-xl font-bold text-green-600">{student.doubt_clearing?.resolved || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  <span className="text-sm">Pending</span>
                  <span className="text-xl font-bold text-orange-600">{student.doubt_clearing?.pending || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  <span className="text-sm">Avg Resolution Time</span>
                  <span className="text-xl font-bold">{student.doubt_clearing?.avg_resolution_time_minutes || 0}m</span>
                </div>
              </div>

              <div>
                <h4 className="font-medium mb-3">Recent Doubts</h4>
                {(student.doubt_clearing?.recent_doubts?.length || 0) > 0 ? (
                  <div className="space-y-3">
                  ...
                  <p className="text-sm text-muted-foreground">No recent doubts.</p>
                )}
              </div>
            </div>
          ) : (
            <div className="text-center py-6">
              <HelpCircle className="h-10 w-10 mx-auto mb-3 text-muted-foreground opacity-50" />
              <p className="text-muted-foreground">No doubt clearing activity yet.</p>
              <p className="text-xs text-muted-foreground mt-1">Ask questions using the AI assistant to see your activity here.</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {hasPodcastData ? (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-3 bg-muted rounded-lg">
                  ...
                  <p className="text-2xl font-bold">{student.podcast_usage?.total_listened || 0}</p>
                </div>
                <div className="p-3 bg-muted rounded-lg">
                  <p className="text-sm text-muted-foreground">Total Time</p>
                  <p className="text-2xl font-bold">{Math.round((student.podcast_usage?.total_time_minutes || 0) / 60)}h</p>
                </div>
                <div className="p-3 bg-muted rounded-lg">
              ...
              {(student.podcast_usage?.recent_podcasts?.length || 0) > 0 && (
                <div>
            ...
            <div className="text-center py-6">
              <Headphones className="h-10 w-10 mx-auto mb-3 text-muted-foreground opacity-50" />
              <p className="text-muted-foreground">No podcast activity yet.</p>
              <p className="text-xs text-muted-foreground mt-1">Listen to AI-generated podcasts to see your activity here.</p>
            </div>
          )}
        </CardContent>
      </Card>
